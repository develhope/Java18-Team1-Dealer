...
public interface ShowroomProfitRepository extends JpaRepository<ShowroomProfit, Long> {
    ...
    @Query(value = "SELECT COUNT * FROM showroom_profit WHERE purchase.salesman.id = :idsalesman AND profit_type_enum LIKE PURCHASE " +
    ...
    @Query(value = "SELECT SUM(profit) FROM showroom_profit WHERE purchase.salesman.id = :idsalesman " +
            "AND pay_date BETWEEN :paydate1 AND :paydate2", nativeQuery = true)
